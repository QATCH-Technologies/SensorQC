...
def apply_scale(image):
    ...
    for x in range(0, width, major_x_spacing):
        for y in range(0, height, major_y_spacing):
            ...
            cv2.putText(image, label, (x + 5, y + 15), font, font_scale, text_color, 1)
    return image
...
class ScanUI(QWidget):
    ...
    def __init__(self):
        ...
        self.rob = Robot()
        # In the ScanUI __init__ method, after initializing self.scope:
        self.current_light_mode = MicroscopeConstants.BRIGHT_FIELD
        self.scope.led_on(MicroscopeConstants.BRIGHT_FIELD)
        self.rob.begin()
    ...
    def create_scan_tab(self):
        ...
        self.move_location_button.clicked.connect(self.move_to_specified_location)
        ...
        self.reset_button.setToolTip("Reset the scan settings and robot position.")
        ...
        self.home_button.setToolTip("Move the robot to the home position (out of way).")
        ...
        # Light Mode Section
        light_mode_group = QGroupBox("Light Mode")
        light_layout = QHBoxLayout()
        self.light_toggle_button = QPushButton("Change lighting mode")
        self.light_toggle_button.setToolTip(
            "Toggle light mode between bright and dark field."
        )
        self.light_toggle_button.clicked.connect(self.toggle_light_mode)
        light_layout.addWidget(self.light_toggle_button)
        light_mode_group.setLayout(light_layout)
        main_layout.addWidget(light_mode_group)
    ...
    def create_control_tab(self):
        ...
        self.position_label = QLabel("Current Position: " + str(self.current_pos))
        ...
        # Main layout for the control tab
        layout = QVBoxLayout()
        layout.addWidget(splitter)

        # Capture Image Button Section
        capture_layout = QHBoxLayout()
        self.capture_image_button = QPushButton("Capture Image")
        self.capture_image_button.setToolTip(
            "Capture the current camera image and save it to a specified location."
        )
        ...
        # Light Mode Toggle Section for Control Tab
        light_control_layout = QHBoxLayout()
        self.light_toggle_button_control = QPushButton("Switch lighting mode")
        self.light_toggle_button_control.setToolTip(
            "Toggle light mode between bright and dark field."
        )
        self.light_toggle_button_control.clicked.connect(self.toggle_light_mode)
        light_control_layout.addStretch()
        light_control_layout.addWidget(self.light_toggle_button_control)
        light_control_layout.addStretch()
        layout.addLayout(light_control_layout)
    ...
    def capture_image(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Captured Image", "", "Images (*.png *.jpg *.bmp)"
        )
        if not file_path:
            self.update_status("Image capture cancelled.")
            return
        ...
        if status:
            ...
            self.update_status(f"Image captured and saved to {file_path}")
        else:
            self.update_status("Failed to capture image.")
    ...
    def toggle_light_mode(self):
        if self.current_light_mode == MicroscopeConstants.BRIGHT_FIELD:
            self.current_light_mode = MicroscopeConstants.DARK_FIELD
            self.scope.led_on(MicroscopeConstants.DARK_FIELD)
            self.light_toggle_button.setText("Switch to BRIGHT_FIELD")
        else:
            self.current_light_mode = MicroscopeConstants.BRIGHT_FIELD
            self.scope.led_on(MicroscopeConstants.BRIGHT_FIELD)
            self.light_toggle_button.setText("Switch to DARK_FIELD")
        self.update_status(f"Light mode set to {self.current_light_mode}")
    ...
    def start_scan(self):
        if not self.scan_in_progress:
            ...
            self.scan_name = (
                f"{batch_name}_{sensor_name}" if batch_name else sensor_name
            )
    ...
    def move_x_positive(self):
        ...
        self.rob.go_to(self.current_pos[0], self.current_pos[1], self.current_pos[2])
        self.update_position_label()
    ...
    def update_position_label(self):
        self.position_label.setText("Current Position: " + str(self.current_pos))
    ...
    def visual_align(self):
        ...
        if not file_name:
            self.update_status(f"No filename provided, {file_name}")
            return
        ...
        while iteration < max_iterations:
            ...
            if offset == (None, None):
                self.update_status("Insufficient feature matches. Trying again...")
                time.sleep(1)
                iteration += 1
                continue
            offset_x, offset_y = offset
            if abs(offset_x) < tolerance and abs(offset_y) < tolerance:
                self.update_status("Alignment within tolerance. Alignment complete!")
                break
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.scan_ui.tabs.setTabIcon(0, self.style().standardIcon(QStyle.SP_FileIcon))
